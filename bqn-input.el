...
(defun bqn--make-bqn-mode-map ()
  (let ((map (bqn--make-base-mode-map bqn-mode-map-prefix)))
    (define-key map (kbd "C-c C-s") 'bqn-interactive-send-region)
    (define-key map (kbd "C-c C-c") 'bqn-interactive-send-current-line)
    (define-key map (kbd "C-c C-l") 'bqn-interactive-send-buffer)
    (define-key map (kbd "C-c C-z") 'bqn-switch-to-interactive)
    map))
...
(defcustom bqn-mode-map-prefix "H-"
